...
// src/utils/get-config.ts
var DEFAULT_COMPONENTS = "@/components/@materials";
var DEFAULT_UTILS = "@/services/tw/utils";
...
async function resolveConfigPaths(cwd, config) {
  ...
  if (config.typescript) {
    ...
    tsConfigPath = path2.resolve(cwd, "./jsconfig.json");
    tsConfig = loadConfig(tsConfigPath);
  }
...
// src/utils/registry/index.ts
var baseUrl = process2.env.COMPONENTS_REGISTRY_URL ?? "https://keen.antoine-libert.com";
...
function getItemTargetPath(config, item, override) {
  ...
  if (!(parent in config.resolvedPaths))
    return null;
  return path3.join(config.resolvedPaths[parent], type);
}
  ...
  try {
    ...
    for (const item of payload) {
      ...
      const files = item.files.map((file) => ({
        ...file,
        path: path4.resolve(targetDir, item.name, file.name)
      }));
      if (!config.typescript) {
...
var diff = new Command2().name("diff").description("check for updates against the registry").argument("[component]", "the component name").option("-y, --yes", "skip confirmation prompt.", false).option(
  ...
  try {
    ...
    if (!options.component) {
      ...
      consola4.log("");
      consola4.info(`Run ${colors2.green("diff <component>")} to see the changes.`);
      process4.exit(0);
    }
    const component = registryIndex.find((item) => item.name === options.component);
    if (!component) {
      consola4.error(`The component ${colors2.green(options.component)} does not exist.`);
      process4.exit(1);
    }
...
var init = new Command3().name("init").description("initialize your project and install dependencies").option("-y, --yes", "skip confirmation prompt.", false).option(
  ...
  try {
    ...
    consola5.log("");
    consola5.info(`${colors3.green("Success!")} Project initialization completed.`);
    consola5.log("");
  } catch (error) {
    handleError(error);
  }
...
async function promptForConfig(cwd, defaultConfig = null, skip = false) {
  ...
  consola5.log("");
  const spinner = ora2("Writing .keen.json...").start();
  const targetPath = path6.resolve(cwd, ".keen.json");
...
async function runInit(cwd, config) {
  ...
  await fs4.writeFile(
    ...
    transformCJSToESM(
      config.resolvedPaths.tailwindConfig,
      config.tailwind.cssVariables ? template(TAILWIND_CONFIG_WITH_VARIABLES)({
        extension,
  ...
  const deps = PROJECT_DEPENDENCIES.base.concat(config.style === "new-york" ? ["@radix-icons/vue"] : ["lucide-vue-next"]).filter(Boolean);
  await Promise.allSettled([
...
async function main() {
  ...
  const program = new Command4().name("keen").description("add awesome components and dependencies to your project").version(packageInfo.version || "1.0.0", "-v, --version", "display the version number");
